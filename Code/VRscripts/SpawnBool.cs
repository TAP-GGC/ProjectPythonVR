...
public class SpawnBool : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        spawnPos = transform;
        spawnBoolOrb();
        this.GetComponent<SpawnBool>().enabled = false;
    }

    private void spawnBoolOrb()
    {
        clone = Instantiate(prefab, spawnPos);
        clone.GetComponent<ValueLogicBool>().rightCon = GameObject.Find("RightHand Controller");
        clone.GetComponent<ValueLogicBool>().leftCon = GameObject.Find("LeftHand Controller");
